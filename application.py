...
# https://www.youtube.com/watch?v=dhHOzye-Rms&t=59s
# https://www.youtube.com/watch?v=6plVs_ytIH8&t=798s
# https://github.com/nachi-hebbar/Flask-Form-Bootstrap/blob/master/python.config




from flask import Flask, render_template, request, flash, redirect, make_response, Response
...
application.secret_key= "neriskasc9245kadfb"
        ...
        while total_run_count < 11:
            ...
            while sil_score != elbow_score:
                try:
                    ...
                    print(elbow_score)
                    yield "data: Elbow calculation = " + str(elbow_score) + "\n"
                    ...
                    yield "data: Silhouette score = " + str(sil_score) + "\n"
                    ...
                    if number_of_runs > 10:
                        test_max = elbow_score * 2 + sil_score * 2
                        number_of_runs = 0
                    Sum_of_squared_distances = []
                    silhouette_coefficients = []
                    number_of_runs += 1
...
@application.route('/process_display' , methods=["GET", "POST"])
def process_display():
    return render_template("process_display.html")
...
def download():
    ...
    if "download" in request.form:
        ...
        resp = make_response(final_df.to_csv())
